...
<template>
  <div>
    ...
    <div class="flex flex-row">
      ...
      <p class="text-2xl text-black dark:text-white font-semibold cityName">Weather for 6 days in {{ cityInput }}</p>
    </div>
    ...
    <div v-if="forecastData.length" class="flex flex-wrap justify-center md:gap-6 mt-2 ml-5">
      <div v-for="forecast in forecastData" :key="forecast.date"
           class="block max-w-sm p-6 mt-2 ml-2 text-black bg-white border border-gray-200 rounded-lg shadow dark:text-white dark:bg-gray-900 dark:border-gray-700 transition-all duration-300">
        ...
        <div class="flex justify-center">
          ...
          <img
            :src="getCustomWeatherIcon(forecast.condition)"
            alt="weather icon"
